...
const fixedShotsDirs =
  fs.existsSync(fixedPath) && fs.lstatSync(fixedPath).isDirectory()
    ? fs
        .readdirSync(fixedPath)
        .filter(dirent =>
          fs.lstatSync(path.join(fixedPath, dirent)).isDirectory()
        )
    : [];
const shotsetDirs = !fixedOnly
  ? fs
      .readdirSync(screenshotsMainPath)
      .filter(
        dirent =>
          fs.lstatSync(path.join(screenshotsMainPath, dirent)).isDirectory() &&
          ["_processed", "_temp_fixed", "_flattened", "_fixed"].indexOf(
            dirent
          ) < 0
      )
  : fixedShotsDirs;
const imageMerge = () => {
  Array.from(shotsetDirs).forEach(rootDir => {
    ...
    const sets = dirents
      .filter(dirent =>
        fs.lstatSync(path.join(rootDirAbs, dirent)).isDirectory()
      )
      .map(dirent => path.join(rootDirAbs, dirent));

    Array.from(sets).forEach(set => {
      const setDirents = fs.readdirSync(set);
      const setPieces = setDirents
        .filter(
          setDirent => !fs.lstatSync(path.join(set, setDirent)).isDirectory()
        )
        .map(setDirent => path.join(set, setDirent));
      const screenshotSet = [];
      const asyncCounter = [];
      ...
      Array.from(setPieces).forEach(shotPiecePath => {
        ...
        if (!match) {
          console.error(
            `The filename of the screenshot piece ${path.basename(
              shotPiecePath
            )} cannot be parsed. The screenshot should be merged manually.`
          );
          return;
        }
        ...
        let offsetY = parseFloat(match[4], 10);
        ...
        Jimp.read(shotPiecePath, (error, piece) => {
          ...
          if (piece.bitmap.height !== height) {
            offsetY *= piece.bitmap.height / height;
          }

          screenshotSet[index - 1] = {
            src: shotPiecePath,
            offsetY: offsetY ? Math.ceil(offsetY) * -1 : 0
          };
