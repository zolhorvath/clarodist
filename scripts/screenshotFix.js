      ...
      if (fixPattern[`${platform}:${browser}`]) {
        ...
        Array.from(setPieces).forEach(shotPiecePath => {
          ...
          Jimp.read(shotPiecePath, (error, piece) => {
            ...
            const pieceFixedName = [
              match[1],
              `${match[2]}x${fixedHeight}`,
              match[4]
            ].join("--");
            const pieceFileName = `${pieceFixedName}.png`;
            ...
            piece
              ...
              .write(path.join(pathComps.join(path.sep), pieceFileName));
          });
        });
      }
    });
