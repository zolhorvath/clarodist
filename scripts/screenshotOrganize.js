...
const imageOrganize = () => {
  Array.from(shotDirs).forEach((rootDir) => {
    ...
    const files = dirents.filter((dirent) => fs.lstatSync(path.join(rootDirAbs, dirent)).isFile()).map(dirent => path.join(rootDirAbs, dirent));
    ...
    Array.from(files).forEach(file => {
      ...
      const fileName = path.join(shotPath, path.basename(file, path.extname(file)) + '--partial' + path.extname(file));
