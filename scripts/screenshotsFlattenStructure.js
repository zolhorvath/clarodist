...
const screenshotsProcessedPath = path.join(
...
const flattener = directory => {
  const subdirs = fs
    ...
    .filter(dirent => fs.lstatSync(path.join(directory, dirent)).isDirectory())
    .map(dirent => path.join(directory, dirent));
  const files = fs
    .readdirSync(directory)
    .filter(dirent => fs.lstatSync(path.join(directory, dirent)).isFile())
    .map(dirent => path.join(directory, dirent));
...
const flattenDirStructure = () => {
  ...
  const testDirs = fs
    .readdirSync(screenshotsProcessedPath)
    .filter(
      dirent =>
        fs
          .lstatSync(path.join(screenshotsProcessedPath, dirent))
          .isDirectory() &&
        ["_processed", "_temp_fixed", "_flattened", "_fixed"].indexOf(dirent) <
          0
    )
    .map(dirent => path.join(screenshotsProcessedPath, dirent));
  ...
  Array.from(testDirs).forEach(testDir => {
    ...
    Array.from(destPathComps).forEach((item, pathCompIndex) => {
      const pathToCheck = destPathComps
